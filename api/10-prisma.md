...
> **En production**, l'application des migrations est gérée automatiquement par `fdn init api`.
...
> En production, ces étapes sont gérées automatiquement par le CLI :
> ```bash
> fdn init api
> ```

Si besoin de les lancer manuellement :
```bash
