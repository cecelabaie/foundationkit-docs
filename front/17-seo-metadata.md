...
export const SITE_CONFIG = {
  name: 'Mon projet',
  title: 'Mon projet : Description courte du projet',
  description: 'Description de mon projet.',
  locale: 'fr_FR',
  ogImage: '/web-app-manifest-512x512.png',
  authors: [{ name: 'Mon projet' }],
};
```
...
Le format du titre généré est automatiquement `${title} | Mon projet` (valeur de `SITE_CONFIG.name`).
...
Il est exécuté automatiquement après `next build` (via `fdn build front`). La configuration se trouve dans `next-sitemap.config.js` à la racine du projet frontend.
